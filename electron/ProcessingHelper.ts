...
export class ProcessingHelper {
  ...
  private isAbortError(error: any): boolean {
    return isCancel(error) || (error instanceof Error && error.name === 'AbortError');
  }
  ...
  public async processScreenshots(): Promise<void> {
    ...
    if (!this.isClientInitialized()) {
      ...
      if (!this.isClientInitialized()) {
        const provider = configHelper.loadConfig().apiProvider;
  ...
  private async handleInitialProcessing(): Promise<void> {
    const mainWindow = this.deps.getMainWindow();
    if (!mainWindow) return;

    mainWindow.webContents.send(this.deps.PROCESSING_EVENTS.INITIAL_START);
    const screenshotQueue = this.screenshotHelper.getScreenshotQueue().filter(p => fs.existsSync(p));
  ...
  private async handleExtraProcessing(): Promise<void> {
    const mainWindow = this.deps.getMainWindow();
    if (!mainWindow) return;

    const extraScreenshotQueue = this.screenshotHelper.getExtraScreenshotQueue().filter(p => fs.existsSync(p));
      ...
      if (config.apiProvider === "openai") {
        ...
        const response = await this.openaiClient.chat.completions.create({
          ...
          messages: [
            ...
            { role: "user", content: [{ type: "text", text: `Extract the coding problem details from these screenshots. Return in JSON format. The target language is ${language}.` }, ...imageDataList.map(data => ({ type: "image_url" as const, image_url: { url: `data:image/png;base64,${data}` } }))] }
          ],
        ...
        const geminiMessages: GeminiMessage[] = [{ role: "user", parts: [{ text: `You are a coding challenge interpreter. Analyze the screenshots of the coding problem and extract all relevant information. Return the information in JSON format with these fields: problem_statement, constraints, example_input, example_output. Just return the structured JSON without any other text. The target language is ${language}.` }, ...imageDataList.map(data => ({ inlineData: { mimeType: "image/png", data } }))] }];
        const response = await axios.post(`https://generativelanguage.googleapis.com/v1beta/models/${config.extractionModel || GEMINI_EXTRACTION_MODEL}:generateContent?key=${this.geminiApiKey}`, { contents: geminiMessages, generationConfig: { temperature: 0.2, maxOutputTokens: 8192 } }, { signal });
        ...
        const response = await this.anthropicClient.messages.create({
          ...
          max_tokens: 4096,
          messages: [{ role: "user", content: [{ type: "text", text: `Extract the coding problem details from these screenshots. Return in JSON format with these fields: problem_statement, constraints, example_input, example_output. The target language is ${language}.` }, ...imageDataList.map(data => ({ type: "image" as const, source: { type: "base64" as const, media_type: "image/png" as const, data } }))] }],
          temperature: 0.2
        }, { signal });
  ...
  private async generateSolutionsHelper(signal: AbortSignal) {
    ...
    if (!problemInfo) return { success: false, error: "Problem information is missing." };
    ...
    try {
        ...
        if (config.apiProvider === "openai") {
            ...
            if (!this.geminiApiKey) return { success: false, error: "Gemini API key not configured." };
            const response = await axios.post(`https://generativelanguage.googleapis.com/v1beta/models/${config.solutionModel || GEMINI_SOLUTION_MODEL}:generateContent?key=${this.geminiApiKey}`, { contents: [{ role: "user", parts: [{ text: promptText }] }], generationConfig: { temperature: 0.2, maxOutputTokens: 8192 } }, { signal });
            responseContent = (response.data as GeminiResponse).candidates?.[0]?.content.parts[0].text;
        } else if (config.apiProvider === "anthropic") {
            if (!this.anthropicClient) return { success: false, error: "Anthropic client not initialized." };
            const response = await this.anthropicClient.messages.create({
  ...
  private async processExtraScreenshotsHelper(
    ...
    if (!problemInfo) return { success: false, error: "Problem info not found for debugging." };
    ...
    try {
      ...
      if (config.apiProvider === "openai") {
        ...
        const response = await this.openaiClient.chat.completions.create({
          ...
          messages: [{ role: "system", content: "You are a debugging assistant." }, { role: "user", content: [{ type: "text", text: debugPrompt }, ...imageDataList.map(data => ({ type: "image_url" as const, image_url: { url: `data:image/png;base64,${data}` } }))] }],
          max_tokens: 4096, temperature: 0.2
        }, { signal });
        debugContent = response.choices[0].message.content;
      } else if (config.apiProvider === "gemini") {
        if (!this.geminiApiKey) return { success: false, error: "Gemini API key not configured." };
        const response = await axios.post(`https://generativelanguage.googleapis.com/v1beta/models/${config.debuggingModel || GEMINI_DEBUGGING_MODEL}:generateContent?key=${this.geminiApiKey}`, { contents: [{ role: "user", parts: [{ text: debugPrompt }, ...imageDataList.map(data => ({ inlineData: { mimeType: "image/png", data } }))] }], generationConfig: { temperature: 0.2, maxOutputTokens: 8192 } }, { signal });
        debugContent = (response.data as GeminiResponse).candidates?.[0]?.content.parts[0].text;
      } else if (config.apiProvider === "anthropic") {
        if (!this.anthropicClient) return { success: false, error: "Anthropic client not initialized." };
        const response = await this.anthropicClient.messages.create({
          model: config.debuggingModel || ANTHROPIC_DEBUGGING_MODEL, max_tokens: 4096,
          messages: [{ role: "user", content: [{ type: "text", text: debugPrompt }, ...imageDataList.map(data => ({ type: "image" as const, source: { type: "base64" as const, media_type: "image/png" as const, data } }))] }],
          temperature: 0.2
        }, { signal });

...
export class ConfigHelper extends EventEmitter {
  ...
  private sanitizeModelSelection(model: string, provider: "openai" | "gemini" | "anthropic"): string {
    if (provider === "openai") {
      ...
      // Note: Updated to allow both pro and flash models
      const allowedModels = ['gemini-1.5-pro', 'gemini-2.0-flash', "gemini-2.5-pro", "gemini-2.5-flash"];
      if (!allowedModels.includes(model)) {
        console.warn(`Invalid Gemini model specified: ${model}. Using default model: gemini-2.5-flash`);
        return 'gemini-2.5-pro'; // Changed default to pro
      }
      return model;
